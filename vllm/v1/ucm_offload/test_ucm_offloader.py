...
def test_ucm_flashmla_deepseekv3_like_smoke():
    ...
    indices_prefill = torch.zeros(
        (batch_size, seqlen_q_prefill, topk_prefill),
        dtype=torch.int32,
        device=device,
    )
    ...
    offloader.prepare_unified_slab(
        per_layer_num_actual_tokens=int(seqlen_q_prefill),
    ...
    slot_mapping = torch.arange(
        79 * block_size,
        (79 + num_new_blocks) * block_size,
        dtype=torch.int32,
        device=device,
    )
    indices_decode = slot_mapping.unsqueeze(0)
    # decode 阶段的 FlashMLA 调用：
    # 这里我们模拟“重新排布后的 block_table + 无 indices”（token 已经 compact）
    ...
    new_tok_ids = topk_indices_global.reshape(-1)  # [2048]
