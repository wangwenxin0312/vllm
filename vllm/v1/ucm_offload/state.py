...
def init_ucm_offloader(
    ...
    *,
    ...
    # 创建/重建单例
    _ucm_singleton = UcmOffloader(vllm_config=vllm_config, role=role, device_id=device_id)
...
def ensure_ucm_offloader_initialized(
    ...
    if offloader is not None:
        return offloader
    return init_ucm_offloader(vllm_config=vllm_config, role=role, device_id=device_id)
