...
class MLACommonImpl(MLAAttentionImpl[M], Generic[M]):
    ...
    def forward(
        ...
        if has_prefill:
            prefill_q, _, _, _ = maybe_execute_sparse_attention_begin(prefill_q, k_c_normed, k_pe, layer.layer_name, forward_context, output, phase="prefill", k_hash=k_hash, k_scale=layer._k_scale)
        ...
        if has_decode:
            ...
            _, _, _, _ = maybe_execute_sparse_attention_begin(torch.cat([decode_ql_nope, decode_q_pe],dim=-1), k_c_normed, k_pe, layer.layer_name, forward_context, output, phase="decode", k_hash=k_hash, k_scale=layer._k_scale, decode_ql_nope=decode_ql_nope, decode_q_pe=decode_q_pe)
